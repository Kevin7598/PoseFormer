...
def train_model(config):
    ...
    elif moduleChoice == 'poseformer':
        from PoseFormer import PoseTransformerV2
        model = PoseTransformerV2(num_classes=vocab_size + 1, num_joints=84, in_chans=3,
    ...
    for epoch in range(1, 200):
        ...
        for batch in tqdm(train_loader, desc=f"[Train] Epoch {epoch}"):
            ...
            with autocast(enabled=False):
                ...
                input_lengths = lengths.clone().detach().to(dtype=torch.int32)
                target_lengths = target_lens.clone().detach().to(dtype=torch.int32)
